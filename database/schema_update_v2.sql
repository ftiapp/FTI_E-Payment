-- FTI E-Payment Database Schema Update v2
-- Update tables to support new form requirements

USE `FTI_E-Payment`;

-- 1. Add first_name and last_name to corporate_customers table
ALTER TABLE `FTI_E-Payment_corporate_customers` 
ADD COLUMN `first_name` VARCHAR(100) NULL AFTER `address`,
ADD COLUMN `last_name` VARCHAR(100) NULL AFTER `first_name`;

-- 2. Add contact_first_name and contact_last_name to personal_customers table
ALTER TABLE `FTI_E-Payment_personal_customers` 
...
-- 4. Make service_or_product nullable in transactions table
ALTER TABLE `FTI_E-Payment_transactions` 
MODIFY COLUMN `service_or_product` VARCHAR(255) NULL;

-- 5. Add original_invoice_number field to transactions table
ALTER TABLE `FTI_E-Payment_transactions` 
...
UPDATE `FTI_E-Payment_corporate_customers` 
SET `first_name` = 'สมชาย', `last_name` = 'ใจดี' 
WHERE `contact_person_name` = 'สมชาย ใจดี';

UPDATE `FTI_E-Payment_personal_customers` 
SET `contact_first_name` = 'มานี', `contact_last_name` = 'รักดี' 
WHERE `first_name` = 'มานี' AND `last_name` = 'รักดี';

-- Show updated table structures
DESCRIBE `FTI_E-Payment_corporate_customers`;
DESCRIBE `FTI_E-Payment_personal_customers`;
DESCRIBE `FTI_E-Payment_transactions`;
